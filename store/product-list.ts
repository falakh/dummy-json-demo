...
export const useProductListStore = create<IProductStore>((set) => ({
    requestProductList:async (query)=>{
        try {
            ...
            set({isLoading:true,products:undefined})
            console.log(deffaultQuery.toString(),query)
            if(!query?.query){
                const productRequest = await fetch(productListUrl+'?'+deffaultQuery.toString())
                const productResponse = await productRequest.json()
                set({isLoading:false,products:productResponse.products,pagination:{
                ...
                set({isLoading:false,products:productSearchResponse.products,
                    pagination:{
